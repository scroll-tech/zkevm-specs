...
class BitshighTable(LookupTable):
    def __init__(self):
        super().__init__(["cell", "bit", "highpow"])
        for idx in range(64):
            for tmpidx in range(8):
                count = 8 * idx + tmpidx
                highpow = 2 ** 64 - 2 ** (64 - count)
                self.add_row(
                    cell=idx,
                    bit=tmpidx,
                    highpow=highpow
...
def shr_sar_common(a: Sequence[U8],
    ...
    for transplacement in range(4):
        ...
        for idx in range(4 - transplacement):
            ...
            print("b_digit:", b_digits[idx])
            print(select_transplacement_polynomial)
    ...
    # highpow_lookup
    assert bitshightable.lookup(
        cell=shift_mod_by_64_div_by_8,
        bit=shift_mod_by_8,
        highpow=high_pow
    )
