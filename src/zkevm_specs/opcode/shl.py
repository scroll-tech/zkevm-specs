...
def check_shl(a: Sequence[U8],
    ...
    assert len(a_slice_front) == len(a_slice_back) == 32
    ...
    for transplacement in range(4):
        ...
        for idx in range(4 - transplacement):
            tmpidx = 3 - (idx + transplacement)
            if tmpidx == 0:
                merge_a = a_slice_back_digits[tmpidx] * shift_mod_by_64_pow
            else:
                merge_a = a_slice_front_digits[tmpidx - 1] + a_slice_back_digits[tmpidx] * shift_mod_by_64_pow
            assert select_transplacement_polynomial * (merge_a - b_digits[3 - idx]) == 0
        for idx in range((4 - transplacement), 4):
            assert select_transplacement_polynomial * b_digits[3 - idx] == 0
